...
function statusOf(t: Todo): { label: string; icon: string; cls: string } {
  if (t.done) return { label: "Completed", icon: "✅", cls: "st-done" };
  if (t.scheduled_date) return { label: "Scheduled", icon: "🗓️", cls: "st-sched" };
  return { label: "New task", icon: "✉️", cls: "st-new" };
}
      ...
      <div className="mt-row">
        ...
        <div className="mt-cell mt-status">
          <span className={`mt-status-pill ${st.cls}`}>
            <span className="mt-status-ic">{st.icon}</span>
            {st.label}
          </span>
        </div>
          ...
          {dueEdit === t.id ? (
            ...
            <button type="button" className="mt-due-btn" onClick={() => setDueEdit(t.id)}>
              {t.due_date ? (
                ...
                <span className="mt-due-empty" aria-label="No due date">🗓️</span>
              )}
            </button>
          )}
        </div>
        ...
        <div className="mt-cell mt-resp">
          <span className="mt-avatar" title={t.assignee ?? undefined}>
